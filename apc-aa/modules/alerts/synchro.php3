 ...
 * @package Alerts
 * @version $Id: synchro.php3 4270 2020-08-19 16:06:27Z honzam $
...
require_once __DIR__."/../../modules/alerts/synchro2.php3";
...
$apage = new AA_Adminpageutil('admin', 'synchro');
$apage->setModuleMenu('modules/alerts');
$apage->setTitle(_m("Synchronization with Reader Management Slice"));
...
echo '
<form method="post" name="form_choose" action="'.StateUrl("synchro.php3").'">
...
if ($slice_set) {
    echo "<a href=\"".StateUrl(AA_INSTAL_PATH."admin/index.php3?change_id=". $collectionprop["slice_id"])."\">".$slice_name.'</a>';
} else {
    echo $slice_name;
}
...
if ($slice_set) {
    echo '<br><input type="checkbox" name="change_to_delete" checked> '
        ._m("and delete the %1-specific fields from %2", [$collectionprop["name"], $slice_name]);
}
...
echo '
<form method="post" name="form_add" action="'.StateUrl("synchro.php3").'">
<table border="1" cellspacing="0" cellpadding="10" align="center" width="440">
    <tr><td class=tabtxt>
    <h2>'._m("Add %1-specific fields to %2",
             [$collectionprop["name"], $slice_name]).'</h2>';
