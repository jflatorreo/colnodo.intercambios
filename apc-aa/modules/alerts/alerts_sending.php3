 ...
 *
 * @package Alerts
 * @version $Id: alerts_sending.php3 4386 2021-03-09 14:03:45Z honzam $
...
require_once __DIR__."/../../include/item_content.php3";
...
class AA_Collection {
    ...
    function _get( $property ) {
        if ( is_null($this->reader_slice_id) ) {
            ...
            $data = DB_AA::select1([], 'SELECT * FROM alerts_collection', [['id', $this->id]]);
    ...
    function getSliceUrl()       {
        if ( is_null($this->slice_url) ) {
            $this->slice_url = DB_AA::select1('slice_url', "SELECT slice_url FROM `module`", [[ 'id', $this->_get('alerts_module_id'), 'l' ]]);
                              //GetTable2Array('SELECT slice_url FROM module WHERE id=\''.q_pack_id($alerts_id).'\'', 'aa_first', 'slice_url');
        }
        return $this->slice_url;
    }

    /**
     * @param null $how_often
     * @return zids
     */
    function getReaders($how_often = null) {
        if (!($slice          = $this->getReaderSlice())) {
            return new zids();
        };
    ...
    function getReadersSelectArray($how_often = null) {
        $ret = [];
        $content4ids = GetItemContent($this->getReaders($how_often), false, true, [FIELDID_USERNAME, FIELDID_EMAIL]);
     ...
     * @param string|array  $emails (email, email, ...)
     ...
     * @param null   $reader_id
     * @return int - count of emails sent
     */
    public function sendEmails($ho, $emails, $update, $item_id, $reader_id) {

        // allow to access Reader data (and the data of all the slices with the same password)
        AA_Credentials::singleton()->loadFromSlice($this->_get('reader_slice_id'));

        $GLOBALS['apc_state']['xlang'] = substr(AA_Module_Alerts::getModule($this->_get('alerts_module_id'))->getLang(),0,2);
        ...
        if ( !is_array($emails) OR !count($emails) ) {
            ...
            AA_Log::write("ALERTS", $ho, "Users for collection " . $this->id . ": " . ((int)$zids->count()));
            ...
            for ( $i=0, $zcount=$zids->count(); $i<$zcount; $i++) {
                ...
                if ($user_text) {
                    ...
                    if (AA_Mail::sendTemplate($this->_get('email_id_alert'), $readerContent->getValue(FIELDID_EMAIL), $item)) {
                        AA_Log::write("ALERTS", $ho, $this->id . ": " . $readerContent->getValue(FIELDID_EMAIL). ' queued');
                        $email_count++;
                    }
                }
            }
        ...
        // Use the emails sent as param
            ...
            AA_Log::write("ALERTS", $ho, "Emails for collection " . $this->id . ": " . ((int)count($emails)));
            $als = new AA_Aliases();
//          $als->addAlias(new AA_Alias("_#FILTERS_", "id..............", 'f_t', array(get_filter_text_4_reader(null, $filters, $this->id))));
            ...
            foreach ( (array)$emails as $email ) {
                if (AA_Mail::sendTemplate($this->_get('email_id_alert'), $email, $item)) {
                    $email_count++;
                }
            }
        }
        AA_Log::write("ALERTS", $ho, "Sent for collection " . $this->id . ": " . ((int)$email_count));
        AA_Log::write("ALERTS", $ho, join('; ', [
            "Debug collection " . $this->id,
            $this->_get('email_id_alert'),
            join(',', (array)$emails),
            join(',', $aliases)
        ]));
        return $email_count;
    ...
    *
    * The only exception are the "instant" and "irregular" messages: If the paramter $item_id
    ...
    */
    function createFilterText($ho, $update, $item_id) {
        ...
        while ($db->next_record()) {
            ...
            $myview["filters"][$db->f("filterid")] = ["conds"=>$db->f("conds")];
            // Group by selections?
            ...
            foreach ($slice["views"] as $vid => $view) {
                foreach ($view["filters"] as $fid => $filter) {
                    ...
                    if ($all_ids) {
                        ...
                        $zids = QueryZids([$slice_id], $conds, $sort, "ACTIVE", 0, new zids( $all_ids,'p' ));
                    }
    ...
    function getFilterText4Reader($readerContent, $filters) {
        ...
        $last_fprop = [];
        $filter_ids = [];  // $filter_ids = "";   // changed to array() for php 7.1 - Jirka
        ...
        foreach ( $filters as $filterid => $fprop ) {
            ...
            if ( $last_fprop["vid"] != $fprop["vid"] AND $filter_ids AND $user_zids->count()) {
                ...
                if ($last_fprop["sort"]) {
                    ...
                    $user_zids = QueryZids([unpack_id($set["info"]->f("slice_id"))], "", $sort, "ACTIVE", 0, $user_zids);
                }

                $user_text .= get_filter_output_cached($last_fprop["vid"], join (",",$filter_ids), $user_zids);

                // $filter_ids = "";   // changed to array() for php 7.1 - Jirka
                $filter_ids = [];
                $user_zids->clear("p");
            }
            if ($fprop == "dummy") {
 ...
 * @param array  $emails (email, email, ...)
 *            If set to "all", all Alerts users are processed.
 * @param bool   $update
 * @param string $item_id
 * @param null   $reader_id
...
function initialize_last() {
    ...
    foreach ($hos as $ho => $v ) {
        ...
        while ($db->next_record()) {
            DB_AA::update('alerts_collection_howoften', [['last',$init[$ho],'i']], [
...
function get_view_settings_cached($vid) {
    ...
    if (!$cached_view_settings[$vid]) {
        $view         = AA_Views::getViewNumeric($vid);
        $slice        = AA_Slice::getModule(unpack_id($view->f("slice_id")));
        $cached_view_settings[$vid] = [
