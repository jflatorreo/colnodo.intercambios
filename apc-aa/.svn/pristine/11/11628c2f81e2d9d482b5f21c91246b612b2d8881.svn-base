...
if (!include_once(__DIR__."/misc/phpThumb/phpthumb.class.php")) {  // Honza AA Change
    ob_end_flush();
    die('failed to include_once("'.__DIR__.'/misc/phpThumb/phpthumb.class.php")');
}
ob_end_clean();
...
ob_start();
include_once __DIR__."/misc/phpThumb/phpthumb.config.php";  // Honza AA Change
ob_end_clean();
...
$allowedGETparameters = ['src', 'new', 'w', 'h', 'wp', 'hp', 'wl', 'hl', 'ws', 'hs', 'f', 'q', 'sx', 'sy', 'sw', 'sh', 'zc', 'bc', 'bg', 'bgt', 'fltr', 'xto', 'ra', 'ar', 'aoe', 'far', 'iar', 'maxb', 'down', 'phpThumbDebug', 'hash', 'md5s', 'sfn', 'dpi', 'sia', 'nocache'];
...
while ($CanPassThroughDirectly && $phpThumb->src) {
    ...
    if ($phpThumb->getimagesizeinfo = @getimagesize($SourceFilename)) {
        ...
        $ImageCreateFunctions = [1=>'ImageCreateFromGIF', 2=>'ImageCreateFromJPEG', 3=>'ImageCreateFromPNG'];
