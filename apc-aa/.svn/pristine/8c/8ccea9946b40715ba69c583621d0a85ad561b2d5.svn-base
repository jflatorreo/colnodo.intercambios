  ...
  * @return mixed */
    ...
    public function __construct() {
        ...
        if (preg_match('/^([a-z]+)\:\/\/([^\/^\:]+)(\:(\d+))?\/(.+)\/?$/',
            ...
            list(, $protocol, $domain, , $port, $path) = $patt;
            ...
            if (isset($this->config['_check4htaccess']) &&
                ...
                if (!file_exists($htaccess)) {
                    ...
                    if (($data != $original) && !@file_put_contents($htaccess, $original))
                        $this->backMsg("Incorrect .htaccess file. Cannot rewrite it!".$htaccess);
                }
            }
...
//       return (substr($rPath, 0, strlen($this->typeDir)).'/' === $this->typeDir); // Jirkare hack
