 ...
 *
 * @version   $Id: item.php3 4409 2021-03-12 13:43:41Z honzam $
...
if ( file_exists( __DIR__."/usr_aliasfnc.php3" ) ) {
  require_once __DIR__."/usr_aliasfnc.php3";
}
...
function GetAliases4Type( $type, $additional="" ) {
    ...
    switch ( $type ) {
        case 'const':
            ...
            $aliases["_#TR_NAME_"] = GetAliasDef( "f__:{tr:{_#NAME###_}}", "const_name",        _m("Translated constant name"));
...
function FillFakeAlias(&$content4id, &$aliases, $alias, $value, $flag=FLAG_HTML) {
    ...
    $content4id[$colname][] = ["value" => $value, "flag" => $flag];
    $aliases[$alias] = GetAliasDef("f_1", $colname, _m("Alias for %1", [$colname]));
}
...
function sess_return_url($url) {
    ...
    return $return_url ? expand_return_url(1) : StateUrl($url);
}
...
function Inputform_url($add, $iid, $sid='', $ret_url='', $vid = null, $var = null, $safe=true) {
    ...
    //$url2go = isset($sess) ? StateUrl($admin_path) : ($admin_path .(isset($AA_CP_Session) ? "?AA_CP_Session=$AA_CP_Session" : "" ));
    ...
    return $safe ? con_url('itemedit.php3',$param) : get_url('itemedit.php3',$param);
}
 ...
 * @return array
 */
function GetFormattedItems($zids, $format, $crypted_additional_slice_pwd=null, $tagprefix=null) {
    $ret  = [];
    if ( $zids->count() <= 0 ) {
        return $ret;
    }
    ...
    foreach($items as $long_id=>$item) {
        $ret[$long_id] = AA::Stringexpander()->unalias($format, '', $item);
    }
 ...
 *        those functions (GetFormattedConstants() and GetFormattedItems())
 *        to itemview
 * @return array
 */
function GetFormatedConstants($constgroup, $format, $restrict_zids, $conds, $sort) {
    ...
    $aliases = GetAliases4Type('const');
    $item    = new AA_Item('',$aliases);
    $format  = $format ?: 'const_name';
    $ret     = [];
 ...
 * @param $content  ItemContent|AA_Content object
 * @return AA_Item
 */
function GetItemFromContent($content) {
    ...
    return new AA_Item($content, new AA_Aliasgenerator_Slice($content->getSliceID()));
}

class AA_Aliasgenerator_Slice {
    protected $slice_id;

    public function __construct(string $slice_id) {
        $this->slice_id = $slice_id;
    }

    public function getAliases() {
        $slice = AA_Slice::getModule($this->slice_id);
        return $slice ? $slice->aliases() : [];
    }
}
...
class AA_Item {
    ...
    protected $aliases_gen; // alias generator for alias lazy load
    var $parameters;        // optional additional parameters - copied from itemview->parameters

    /** item function
     *  Constructor
     *  @param $content4id ItemContent|AA_Content|zids|array - could be ItemContent as well as zids as well as old
     *                     contentent array
     *  @param $aliases
     *  @param $format
     *  @param $remove
    ...
    function __construct($content4id='', $aliases='', $format='', $remove='', $param=false){
        ...
        if (is_object($aliases)) {
            $this->aliases = null;
            $this->aliases_gen = $aliases;
        } else {
            $this->aliases = $aliases;
            $this->aliases_gen = null;
        }
        $this->format       = $format;
        $this->remove       = $remove;
     ...
     * @return bool
     */
    function getval($field_id, $index=0) {
        return $this->content4id->getValue($field_id, $index);
    }
    ...
    function isEmpty() { return $this->content4id->isEmpty(); }

    /** Without $bin returns true if the item is viseble - not expired/trashed/pending/...
    ...
    function isActive($bin=AA_BIN_ACTIVE) {
        return $this->content4id->isActive($bin);
    }

    function getStatusCode() {
        return $this->content4id->getStatusCode();
    }
    ...
    /** col2Html function - better txt2html function - it replaces also urls...
     * @param $col
     * @return string
     */
    function col2Html($col) {
        if (!strlen($txt = $this->getval($col)) OR (($flag=$this->getFlag($col)) & FLAG_HTML)) {
            return $txt;
        }
        return nl2br(str_replace('  ', ' &ensp;', AA_Stringexpand_Text2html::htmlEscapeAndLinkUrls($txt)));
    }
    ...
    function getbaseurl($redirect, $no_sess) {
        ...
        $url_base = ( $redirect ?: (($no_sess OR !is_object($sess))  ? '' : StateUrl('')) );
    ...
    function getitemurl($extern, $extern_url, $redirect, $condition=true, $no_sess=false) {
        if ( $extern ) {      // link_only
            return ($extern_url ?: NO_OUTER_LINK_URL);
        }
        if ( !$condition ) {
            return false;
        }
    ...
    protected function loadAliases() {
        if (is_null($this->aliases)) {
            $this->aliases = $this->aliases_gen ? $this->aliases_gen->getAliases() : [];
        }
    }

    /**
     *  get array of alias names
     */
    public function getAliasNames() {
        is_null($this->aliases) && $this->loadAliases();
    ...
    function getahref($url, $txt, $add="", $html=false, $hide=false) {
        if ( $url AND $txt ) {
            ...
            if ($hide=='1') {
                $linkpart=explode('@',str_replace("'", "\'", $url.'@'.$this->text2Html($txt, $html)),4);
                $link  = $linkpart[0]."'+'@'+'".$linkpart[1];
                $inner = $linkpart[2] . ($linkpart[3] ? "'+'@'+'".$linkpart[3] : '');
                if (!$add) {
                    // new approach - do not support $add
                    $ret = "<script>var el=document.createElement('a');el.setAttribute('href','$link');el.innerHTML='$inner';var scrs=document.getElementsByTagName('script');var scr=scrs[scrs.length-1];scr.parentNode.insertBefore(el,scr);</script>";
                } else {
                    // old, but hart to replace by createElement
                    $ret = "<script>\n document.write('<a href=\"".$linkpart[0]."'+'@'+'".$linkpart[1]."\" ".$add.">". $inner ."</a>')\n</script>";
                }
                return $ret;
            } else {
                return '<a href="'. $url ."\" $add>". $this->text2Html($txt, $html).'</a>';
            }
        }
        return $this->text2Html($txt,$html);
    }
     ...
     */
    function get_alias_subst( $alias, $default=null, $use_field="" ) {
        is_null($this->aliases) && $this->loadAliases();
        ...
        if ( !is_array($ali_arr) ) {
            ...
            if ( ! ((substr($alias,9,1)=='#') AND is_array($ali_arr = $this->aliases[substr($alias,0,9).'_']))) {
                return is_null($default) ? $alias : $default;
            }
        }
        ...
        if (!is_callable([$this,$fce])) {
            return _m('Error: wrong alias function %1 for %2', [$fce,$alias]);
        }
        ...
        AA::$debug&8 && AA::$dbg->tracestart($fce, $param);
        $ret = $this->$fce($field_id, $param);
        AA::$debug&8 && AA::$dbg->traceend($fce, $ret);
        return $ret;
    }
    ...
    function unalias( $text, $remove="" ) {
        return AA::Stringexpander()->unalias($text, $remove, $this);
    }
    ...
    /** f__ function - just code
     *  prints the code in parameter (no implicit output of the content of the field)
     *   - _#this could be used
     *   - easier than f_t (no additional parameter)")
     * @param $col
     * @param $param string to be printed (like <img src="{img_src........1}"></img>
     * @return bool|mixed|null|string|string[]
     */
    function f__($col, $param="") {
    ...
    function f_2($col, $param="") {
        ...
        if (!strlen($txt = $this->getval($col, AA_Langs::getLangName2Num(AA::$lang)))) {
            // if not present - translate to default language of the slice
            $translations = AA_Slice::getModuleProperty($this->getSliceID(),'translations');
            if (!strlen($txt = $this->getval($col, AA_Langs::getLangName2Num($translations[0])))) {
                // last try the 0 index (if the field was not set to translation)
                $txt = $this->getval($col);
            }
        }
        return $this->text2Html($txt, $this->getFlag($col));
    }


    /** Prints the parameter - it is shortcut for f_t:<text>:asis
     *  Internal function - used just for addTextAlias()
     * @param string $col
     * @param string $param
     * @return string
     */
    function f_p($col, $param="") {
        return str_replace('#:',':',$param);  // slower, but using the right code path: join(':',ParamExplode($param));
    }
    ...
    function f_h($col, $param="") {
        if ( empty($param) ) {
            return $this->text2Html($this->getval($col), $this->getFlag($col));
        }
        ...
        $res = '';
        foreach ( $values as $v ) {
            $res .= ($res ? $param : ''). $this->text2Html($v['value'], $v['flag']); // add value separator just if field is filled
        }
        return $res;
    }
    ...
    function f_d($col, $param="") {
        ...
        // we check, if the value is not so big (because we solved problem, when
        ...
        $dstr = date($param, (int)max(-2147483647,min(100000000000,$this->getval($col))));
    ...
    function f_y($col, $param="") {
        $str2unalias = $param ? $param : $this->text2Html($this->getval($col), $this->getFlag($col));
    ...
    function i_s($col, $param="") {
        ...
        [ $ptype ] = $this->subst_aliases( ParamExplode($param) );
        if ( !in_array($ptype, ['width', 'height', 'imgtype', 'mime', 'html', 'htmlb']) ) {
            $ptype = 'hw';
        }
        return StrExpand('AA_Stringexpand_Img', [$this->getval($col), '', $ptype]);
    }
    ...
    function f_a($col, $param="") {
        [         , $field ]               = ParamExplode($param);  // we need $field unexpanded
        [ $plength, $pfield, $pparagraph ] = $this->subst_aliases( ParamExplode($param) );
        $value = $this->getval($col);
        if ($value AND !($col == $field)) {  // special case - return whole field
            return $this->text2Html( $value, $this->getFlag($col) );
        }

        if ($pparagraph) {
            $shorted_text = StrExpand('AA_Stringexpand_Shorten', [get_if($value, $pfield), $plength]);
        } else {
    ...
    function f_f($col, $param="") {
        [$plink, $predir, $psess] = $this->subst_aliases(ParamExplode($param));
    ...
    function f_b($col, $param="") {
        ...
        [$plink_only, $purl_field, $predirect, $ptxt, $pcondition, $paddition, $pno_sess] = $this->subst_aliases($p);
    ...
    */
    ...
    function f_t($col, $param="") {
        ...
        if ( isset($p[1]) ) {
            ...
            $modif = $p[1];
            if (in_array($modif, ['csv', 'safe', 'javascript', 'urlencode', 'striptags', 'rss', 'conds', 'asis', 'substitute', 'debug'])) {
                if ( $fnc = AA_Stringexpander::$php_functions[$modif] ) {
...
//                return call_user_func( array( AA_Stringexpand::constructClassName($modif), 'expand'), $text);
        ...
        return strlen($param) ? $this->subst_alias( $param ): $this->text2Html($this->getval($col), $this->getFlag($col));
    }
    ...
    function f_l($col, $param="") {
        [$plink, $padditional] = $this->subst_aliases( ParamExplode($param) );
    ...
    function f_e($col, $param="") {
        ...
        switch( $p[0]) {
            ...
            case "disc":
                ...
                return con_url(StateUrl("discedit.php3"), "item_id=". $this->getItemID());
            ...
            case "slice_info":
                return StrExpand('AA_Stringexpand_Modulefield', [get_if($this->getSliceID(),$GLOBALS['slice_id']), $col]);
            case "link_edit":
                return (($p[1]=='anonym') ?
    ...
    function f_c($col, $param="") {
        ...
        [$pcond, $pbegin, $pend, $pnone, $pccol, $pskip_col] = $this->subst_aliases($p);
        ...
        if (!$pskip_col) {
            $coltxt = $this->text2Html($this->getval($col), $this->getFlag($col));
        }
    ...
    function f_v($col, $param="") {
        ...
        return (new AA_Showview(ParseViewParameters($this->subst_alias($param))))->getViewOutput();
    }
    ...
    function f_m($col, $param="") {
        $p = ParamExplode($param);
        [$pbegin, $pfield, $pelse, $ptype, $padd, $phide] = $this->subst_aliases($p);
    ...
    function f_j($col, $param="") {
        $p = ParamExplode($param);
        [$start, $n, $case, $addstr] = $this->subst_aliases($p);
    ...
    function f_z($col, $param="") {
        return StrExpand('AA_Stringexpand_Fileinfo', [$this->getval($col),$param]);
    }
    ...
    function l_p($col, $param="") {
        ...
        $translate = AA::Contentcache()->get_result( 'GetTable2Array', ["SELECT id, name FROM links_categories WHERE deleted='n'", 'id', true]);

        [$start, $format, $separator, $catseparator, $urlprm, $url_base, $restrict] = $this->subst_aliases(ParamExplode($param));
        if ( !$separator ) {
            $separator = ' &gt; ';
        }
        ...
        $categs2print = $catseparator ? $this->getValues($col) :
                                        [0=> ['value'=>$this->parameters['category_id']]]; // current category
...
/**
 * Class AA_Items
 * @method  static AA_Items singleton()
 */
class AA_Items {
     ...
     *  Array short item id -> AA_Item object (short for smaller memmory usage)
     */
    protected $_i = [];
     ...
     *
     *  This function is used for caching all the items which we need during
     *  page rendering. The main reason is, that we want to grab the item from
     *  database just once during the page creation.
     *
     *  Creates item object just from item id and fills all necessary structures
     * @param  $zid zids    - an item id - zid object, unpacked or short id
     * @param  $crypted_additional_slice_pwd string
     * @param  $renew bool  - regenerate the item form database
     * @return AA_Item|bool|mixed|null
     */
    static function getItem($zid, $crypted_additional_slice_pwd=null, $renew=false) {
        if (empty($zid)) {
            return false;
        }
    ...
    /** @param zids $zids
     * @return AA_Item[] - array(long_id -> AA_Item)
     */
     ...
     * @param $bin - AA_BIN_ACTIVE or AA_BIN_HOLDING or AA_BIN_TRASH or AA_BIN_ALL or its bitwise combination (the trash and pending is not supported, now)
     * @return array
     */
    static function getFormatted($zids, $expression, $bin=AA_BIN_ACTIVE) {
        $res = [];
        ...
        for ( $i=0, $ino=$zids->count(); $i<$ino; ++$i) {
            $item = $items->_getFromCache($zids->zid($i));
            if ($item AND (($bin==AA_BIN_ALL) OR $item->isActive($bin))) {  // AA_BIN_ALL - all items - speedup - no need to check
                $res[$item->getItemId()] = strlen($expression) ? $item->subst_alias($expression) : '';       // long item id
            }
        }
        return $res;
    }
    ...
    static public function deleteItems($zids) {
        ...
        $short_arr = $zids->shortids();
        DB_AA::delete_low_priority('item',        [['id',             $ids_arr,   'l']]);
        DB_AA::delete_low_priority('content',     [['item_id',        $ids_arr,   'l']]);
        DB_AA::delete_low_priority('discussion',  [['item_id',        $ids_arr,   'l']]);
        DB_AA::delete_low_priority('offline',     [['id',             $ids_arr,   'l']]);
        DB_AA::delete_low_priority('relation',    [['source_id',      $ids_arr,   'l'], ['flag', REL_FLAG_FEED, 'set']]);
        DB_AA::delete_low_priority('relation',    [['destination_id', $ids_arr,   'l'], ['flag', REL_FLAG_FEED, 'set']]);

        DB_AA::delete_low_priority('hit_archive', [['id',             $short_arr, 'i']]);
        DB_AA::delete_low_priority('hit_short_id', [['id',             $short_arr, 'i']]);
        DB_AA::delete_low_priority('hit_long_id', [['id',             $ids_arr,   'l']]);

        $ctx = AA_Log::context();
        AA_Log::write("ITEM_DEL", array_map(function($id) use ($ctx) {return "$id:$ctx";}, $ids_arr));

        ChangesMonitor::deleteChanges($ids_arr);
    }
    ...
    /** @returns AA_Item or null (if not cached)
     * @param $zid zids
     * @return mixed|null
     */
    function _getFromCache($zid) {
        if ( $zid->use_short_ids() ) {
            ...
            return isset($this->_i[$id]) ? $this->_i[$id] : null;
        }
