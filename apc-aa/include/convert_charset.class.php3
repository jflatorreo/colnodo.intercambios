...
define ('CONVERT_TABLES_DIR', __DIR__."/ConvertTables/");
define ('DEBUG_MODE', -1);
...
class ConvertCharset {
    ...
    /** Called like $encoder = ConvertCharset::singleton($from, $to)
     * @return ConvertCharset
     */
    static function singleton($from_charset = null, $to_charset = null) {
    ...
    function HexToUtf($UtfCharInHex) {
        ...
        elseif ($UtfCharInDec<2048)   $OutputChar .= chr(($UtfCharInDec>>6)+192). chr(($UtfCharInDec&63)+128);
        elseif ($UtfCharInDec<65536)  $OutputChar .= chr(($UtfCharInDec>>12)+224).chr((($UtfCharInDec>>6)&63)+128). chr(($UtfCharInDec&63)+128);
        elseif ($UtfCharInDec<2097152)$OutputChar .= chr($UtfCharInDec>>18+240).  chr((($UtfCharInDec>>12)&63)+128).chr(($UtfCharInDec>>6)&63+128). chr($UtfCharInDec&63+128);
        return $OutputChar;
    }
     ...
     * @param bool $dot        - allow dot in the escaped sequence
     * @return string
     */
    function escape($string, $encoding=null, $dot=false) {
        ...
        $base  = trim(preg_replace($dot ? '/[^\da-z-_.]/i' : '/[^\da-z-_]/i', '-', $base), '-.');
        $count = 1;
        while ( $count ) {
            $base = str_replace(['--','..','.-','-.'], ['-','.','.','.'], $base, $count);
        }
    ...
    function Convert($StringToChange, $FromCharset = null, $ToCharset = null, $TurnOnEntities = false) {
        ...
        if (($FromCharset == 'utf-8') AND ($ToCharset == 'us-ascii')) {
            ...
            // the strtr and the $table is workaround for systems with broken iconv (RHEL 4 with glibc 2.3.x), where iconv gives bad result just with questionmarks
            $table = [
                ...
                'ÿ'=>'y', 'Ŕ'=>'R', 'ŕ'=>'r', 'Ě'=>'E', 'ě'=>'e', 'ř'=>'r', 'Ř'=>'R', 'ť'=>'t', 'Ť'=>'T', 'Ď'=>'D',
                'ď'=>'d', 'ň'=>'n', 'Ň'=>'N', 'ů'=>'u', 'Ů'=>'U'
            ];

            return iconv("utf-8", "us-ascii//TRANSLIT", strtr($StringToChange, $table));
        }
